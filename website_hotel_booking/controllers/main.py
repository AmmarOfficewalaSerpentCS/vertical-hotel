...
class PaypalController(odoo.addons.payment_paypal.controllers.main.PaypalController):
    ...
    def paypal_validate_data(self, **post):
        ...
        paypal_urls = request.env['payment.acquirer']._get_paypal_urls(tx and tx.acquirer_id and tx.acquirer_id.environment or 'prod')
        ...
        if resp == 'VERIFIED':
            ...
            res = request.env['payment.transaction'].form_feedback(post, 'paypal')
            # change state in transaction
            change_state = request.env['payment.transaction'].write(int(post.get('new_transaction_id')),{'state':'done'})
            reservation_id = request.env['hotel.reservation'].search([('reservation_no', '=', str(post.get('item_number')))])
            #Create new Move
            create_new_move = request.env['hotel.reservation'].confirmed_reservation(reservation_id)
            create_new_move_folio = request.env['hotel.reservation']._create_folio(reservation_id)
...
#         cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        ...
        resv_id = request.env['hotel.reservation'].sudo().search([('reservation_no', '=', reference)])
        if resv_id:
        # Create new transaction in odoo
            ...
            post.update({'new_transaction_id': new_transaction_id, 'new_transaction_name': reference})
...
class Websitehotelbooking(http.Controller):
    ...
    def get_data(self, *args, **kwargs):
        ...
        company_id = request.env['res.users'].sudo().browse(request.uid).company_id
    ...
    def park_details(self,**post):
        if post.get('park_id') :
            ...
            partner_rec = request.env['res.partner'].sudo().search([('id','=',company_id),('active','=',True)])
    ...
    def check_availability(self , **post):
        ...
        room_type = request.env['product.category'].sudo().search([('id', '=', post.get('room_types'))])
        if post.get('park_id') :
            hotel_room_ids = request.env['hotel.room'].sudo().search([('isroom', '=', True),('partner_id.id','=', int(post.get('park_id')))])
            ...
            for room in hotel_room_ids:
                ...
                if not assigned:
                    ...
                    obj = request.env['hotel_reservation.line'].sudo().search([('id', 'in', room_ids)])
            if reserved_rooms:
                reserved_hotel_room = hotel_room_ids.sudo().search([('id','in',reserved_rooms)])
                values.update({'reserved_rooms':reserved_hotel_room})
            if unreserved_rooms:
                unreserved_hotel_room = hotel_room_ids.sudo().search([('id','in',unreserved_rooms)])
            ...
            acquirer_id = payment_acquirer_obj.sudo().search([('name', '=', 'Paypal')])
            base_url = payment_acquirer_obj.env['ir.config_parameter'].get_param('web.base.url')
    ...
    def book_now(self, **post):
        ...
        selected_rooms = request.env['hotel.room'].sudo().search([('id', 'in', selected_room_ids)])
        ...
        if post.get('app_adults') or post.get('app_child'):
            ...
            if post.get('app_adults') and post.get('app_child'):
                total_guest = int(post.get('app_adults')) + int(post.get('app_child'))
    ...
    def create_partner(self,**post):
        partner_ids = request.env['res.partner'].sudo().search([('email','=',post['app_email'])])
        vals = {
        ...
        if not partner_ids:
            ...
            room_types_id = request.env['product.category'].sudo().search([('id','=',rooms_types)])
...
#                          'reservation_line':  [[0, 0, {'categ_id': room_types_id.id, 'reserve': [[6, 0, post.get('selected_rooms')]]}]]
            ...
            if pid:
                vals_hotel_rec.update({'pricelist_id': pid.id})
        get_checkin_date = datetime.strptime(post.get('arrival_date'),"%d/%m/%Y %H:%M:%S")
        get_checkout_date = datetime.strptime(post.get('depature_date'),"%d/%m/%Y %H:%M:%S")
        vals_hotel_rec.update({
        ...
        hotel_reservation_id = request.env['hotel.reservation'].sudo().create(vals_hotel_rec)
        vals_reservation = {
                            'partner_id': partner_ids.id or res_partner.id,
